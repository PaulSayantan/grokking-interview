...
- **When to buy provisioned:** sustained production load that hits on-demand quotas, strict
  latency SLAs, or you must serve a **custom fine-tuned model** (no on-demand option).
...
**Retrieval enhancements:** **hybrid search** (fuse BM25 keyword score with vector score to
catch exact terms/IDs that embeddings miss), metadata **pre-filtering** (tenant, date, ACL),
and **re-ranking** (below).
...
- **Quantization (INT8/FP8/AWQ/GPTQ)** shrinks the model to fit smaller/fewer GPUs and speeds
...
| **Fine-tuning** | behavior, style, format, narrow tasks | high (data + training + must use Provisioned Throughput) | low (retrain to update) | consistent format/tone, domain skill, latency |
...
pre-training** for supported models; the resulting **custom model requires Provisioned
Throughput** to serve. SageMaker JumpStart / training jobs handle fine-tuning open models
(LoRA/QLoRA for parameter-efficient, cheaper tuning).
...
- **Fine-tuning cost trap:** training cost + mandatory Provisioned Throughput to serve means
...
The recurring decisions and the crisp answer:
