...
```mermaid
flowchart TD
    A[Strong loop completed] --> B{Level confirmed<br/>and correct?}
    B -- No / down-leveled --> C[Ask for the evidence gap;<br/>push back with new evidence<br/>or reconsider walk-away]
    B -- Yes --> D[Receive verbal offer]
    C --> D
    D --> E[Do NOT accept on the spot]
    E --> F[Get full breakdown in writing:<br/>base, bonus, equity + vesting, sign-on]
    F --> G[Benchmark TC vs levels.fyi<br/>for level and market]
    G --> H{Below market or<br/>competing offer?}
    H -- Yes --> I[Counter respectfully with data<br/>on flexible levers: equity, sign-on]
    H -- No --> J[Evaluate fit + red flags<br/>from reverse questions]
    I --> J
    J --> K[Decide by your stated date;<br/>confirm final terms in writing]
```
