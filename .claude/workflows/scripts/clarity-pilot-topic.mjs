...
const WHY = [
  ...
  '',
  'The feedback was then sharpened, and this is the bar that outranks the rest:',
  '',
  '> "the explanations shouldn\'t be clumsy or lazy that students have to ponder on the internet again to understand the concept from other different sources."',
  '',
  '**A topic is understandable from the file alone.** That is rule **C12** in the standard, and it is the one most likely to be violated by accident: plainer sentences make it easy to move a hard link out of the prose and into a citation, a footnote or a think-prompt. Prompts, References and the cliffhanger may all send a reader BEYOND the file. None of them may stand in for an explanation the file owes. **Length is not the constraint** — the user has accepted ~80% growth explicitly. Closing a gap is never bloat.',
  '',
...
const written = await agent(
  [
    ...
    '- **The REGISTER SWAP is the binding gate.** Read each section first paragraph and its last back to back; if a reader could tell which is which from vocabulary and clause depth alone, the deep tier failed. Apply it to yourself before finishing. Sample THREE paragraphs per section, not two — A the opener, B the deep beat FIRST body paragraph, C its last — and fail if EITHER pair is distinguishable. A previous writer scored itself 11/11 PASS while the two densest paragraphs in the file sat in B, unmeasured.',
    '- **C12 — the closure ladder, and it is the reason this pass exists.** Enumerate the closure set (each H2 first body line, each section exit sentence problem, every KEY-TAKEAWAY and WARNING assertion, the opener question, and every prompt expected answer). For each, ask "why?" up to three times. Every step must be closed BY THIS FILE, or terminate in one of exactly three floors: a claim the ledger records as already established, an entry on `assumed_prior_knowledge`, or a requirement the file STATES (a bare citation is provenance, not explanation — "per the OCI spec, X" is an unclosed step wearing a citation). Report the table: claim, depth reached, floor. Zero unclosed steps.',
    '- **Prompts are provocation, never carriage.** Delete every prompt and the topic must still teach itself completely. No tier-C `success_criterion` may carry the definition, magnitude or condition of anything the prose uses as a REASON. No caveat, boundary or number may exist only in prompts.yaml. Same test for the cliffhanger: it withholds the NEXT topic payoff, never a link in this topic own chain.',
  ...
  ].join('\n'),
  {
    ...
    schema: {
      ...
      properties: {
        ...
        register_swap: { type: 'string', description: 'All THREE lists (A opener / B deep-beat FIRST paragraph / C deep-beat last), quoted briefly, plus your honest verdict on BOTH pairs. Reporting only two lists is itself a finding.' },
        closure_ladder: { type: 'string', description: 'The C12 table: each closure-set claim, the depth its why-ladder reached, and its floor (ledger / prior / stated). Name any step you could NOT close.' },
        punt_hits: { type: 'integer', description: 'c12_punt_hits from `clarity_report.py --file` on YOUR rewritten file' },
        prompts_subordinate: { type: 'string', description: 'Confirm the topic still teaches itself with every prompt deleted, and name the closure-set claim each tier-C/D prompt interrogates' },
      ...
      required: ['files_written', 'h2_verification', 'word_delta', 'register_swap', 'closure_ladder', 'punt_hits', 'prompts_subordinate', 'added_claims_count', 'cliffhanger', 'prompts_summary'],
    },
  }
)
  ...
  () =>
    agent(
      [
        ...
        '2. **C12 — CLOSURE (re-run it independently; this is the reason the whole pass exists).** The bar the reader feedback set: *"the explanations shouldn\'t be clumsy or lazy that students have to ponder on the internet again to understand the concept from other different sources."* Take the closure set yourself (each H2 first body line, each exit sentence problem, every KEY-TAKEAWAY and WARNING, the opener question, every prompt expected answer). Ask "why?" up to three times per claim. **Name every step the FILE does not close** — each one must instead terminate in a claim the ledger records as established, an entry on the ledger `assumed_prior_knowledge` list, or a requirement the file STATES. A bare citation standing in for the requirement it cites is an unclosed step. Then the subordination test: **read the file with every prompt and the cliffhanger deleted — is any claim now unfollowable?** If a tier-C `success_criterion` carries a definition, magnitude or condition the prose uses as a reason, that is a blocker and the fix is the prose sentence, not a better prompt. Do not accept the writer `closure_ladder` table; build your own and say where they differ.',
        '3. **Anti-dumbing-down.** Hard claims softened, edge cases dropped, numbers vagued, nuance hedged away. Any instance is a blocker.',
        '4. **MCQ survivability.** Read ' + TDIR + '/questions.yaml. **Can every question still be answered from the rewritten prose?** This is the concrete test of no-information-loss and the reason the anchors are frozen. Name any question that lost its support.',
        '5. **UNDISCLOSED ADDED CLAIMS.** Align sentences old-to-new yourself. Find claims missing from audit.md — especially ones with NO marker word, the class that defeated the grep four times. Say whether each is true, false or unverifiable, and how you know.',
        '6. **Information inventory.** Independently diff numbers with units, versions, citations, inline-code identifiers, table rows, code fences, mermaid blocks, callouts.',
        '7. **H2 integrity.** Verify byte-identical H2 text with read_headings; confirm every questions.yaml ref still resolves.',
        '8. **prompts.yaml.** Parse it. Kinds/tiers/caps, one prompt per anchor, every ref and answer_in resolves, payoff resolves in the genuine next topic and that topic really contains it. Cliffhanger form limits, with measured figures.',
        '9. **Continuity.** Canonical terms honoured? Running example reused rather than duplicated? Does ledger-append accurately describe the file?',
        '10. **Bloat.** Growth carrying information, or just chattier? Quote any padding.',
        '',
