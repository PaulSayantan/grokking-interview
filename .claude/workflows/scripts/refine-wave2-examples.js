export const meta = {
  name: 'refine-wave2-examples',
  description: 'WAVE 2 of corpus refinement: the worked-examples + intuition pass over the example-gap files (example_score <= 3) in the remaining 14 domains (java-jvm, messaging-databases, kubernetes, spring-core, observability, grpc, networking, docker, security, interview-craft, hibernate-jpa, rest-api-design, reliability-ops, testing). Cache-backed and idempotent: each file writes a done-marker after self-verification and is skipped on re-runs, so outage retries are additive. One agent per file (distinct files -> safe parallel in-place edits) ADDS concrete numbers-in/numbers-out worked examples for each hard concept (plus intuition-first openers, reasoned trade-offs, and a gotchas/follow-ups turn where the audit flagged them), following the repo refining-content standard. A second adversarial agent per file RECOMPUTES every new worked example to confirm it yields the stated answer and checks for regressions (new wrong claims, broken anchors, contradictions). EDITS concepts.md; never renames headings or touches questions.yaml.',
  phases: [
...
const REFINE_CACHE = '/Users/dev/.claude/projects/<this-repo-project-dir>/refine-cache/wave2'
...
log(`Wave 2 worked-examples refine: ${WORK.length} files; done-markers in ${REFINE_CACHE}`)
...
log(`Wave 2 done: ${clean.length} clean, ${rework.length} need-rework, ${noChange.length} no-change, ${blocked.length} blocked; ${totalExamples} worked examples added`)
